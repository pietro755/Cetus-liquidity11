...
// ---------------------------------------------------------------------------
// topUpLiquidityIfNeeded — top-up when new position liquidity is below target
// ---------------------------------------------------------------------------

describe('rebalance – top-up liquidity when new position is below close_position amount', () => {
  afterEach(() => { delete process.env.DRY_RUN; });

  /**
   * Helper that builds the full rebalance mock scaffold and returns the
   * `createAddLiquidityFixTokenPayload` spy so callers can assert on it.
   */
  function makeTopUpScenario(opts: {
    newPositionLiquidity: string;
    topUpBalanceA: string;
    topUpBalanceB: string;
  }) {
    process.env.DRY_RUN = 'false';
    ...
    // Simulates what the chain returns when topUpLiquidityIfNeeded fetches the new position.
    const newPos = {
      positionId: '0xnewpos',
      poolAddress: '0xpool',
      tickLower: 400,
      tickUpper: 600,
      liquidity: opts.newPositionLiquidity,
      tokenA: '0xcoinA',
      tokenB: '0xcoinB',
      inRange: true,
    };
    ...
    (monitor.isPositionInRange as jest.Mock).mockReturnValue(false);
    // Call order:
    //   1st call – checkAndRebalance initial position fetch
    //   2nd call – removeLiquidity retry-loop re-fetch
    //   3rd call – topUpLiquidityIfNeeded position lookup
    (monitor.getPositions as jest.Mock)
      .mockResolvedValueOnce([pos])
      .mockResolvedValueOnce([pos])
      .mockResolvedValueOnce([newPos]);
    ...
    const mockSuiClient = {
      signAndExecuteTransaction: jest.fn()
        ...
        // 3: initial addLiquidity
        .mockResolvedValueOnce({ effects: successEffect, digest: '0xadd' })
        // 4: top-up addLiquidity (only used when top-up is triggered)
        .mockResolvedValueOnce({ effects: successEffect, digest: '0xtopup' }),
      // getBalance call order:
      //   1-2: pre-removal snapshot  → 0
      //   3-4: post-removal snapshot → received amount
      //   5-6: top-up getWalletBalances
      getBalance: jest.fn()
        .mockResolvedValueOnce({ totalBalance: '0' })          // 1: pre A
        .mockResolvedValueOnce({ totalBalance: '0' })          // 2: pre B
        .mockResolvedValueOnce({ totalBalance: '1000000' })    // 3: post A
        .mockResolvedValueOnce({ totalBalance: '1000000' })    // 4: post B
        .mockResolvedValueOnce({ totalBalance: opts.topUpBalanceA }) // 5: top-up A
        .mockResolvedValueOnce({ totalBalance: opts.topUpBalanceB }), // 6: top-up B
      getObject: jest.fn().mockResolvedValue({ data: { objectId: '0xnewpos', type: 'position' } }),
    };
    ...
    return { pool, config, monitor, mockSdk, mockSuiClient, sdkService, createAddLiquidityFixTokenPayload };
  }

  it('calls createAddLiquidityFixTokenPayload a second time when new position liquidity is below storedLiquidity', async () => {
    const { sdkService, monitor, config, createAddLiquidityFixTokenPayload } = makeTopUpScenario({
      newPositionLiquidity: '4000000', // below storedLiquidity of 5000000
      topUpBalanceA: '500000',
      topUpBalanceB: '500000',
    });
    ...
    // createAddLiquidityFixTokenPayload must be called TWICE:
    // once for the initial deposit, once for the top-up.
    expect(createAddLiquidityFixTokenPayload).toHaveBeenCalledTimes(2);

    // The top-up call must target the new position.
    ...
    expect(typeof topUpArgs.fix_amount_a).toBe('boolean');
  });

  it('skips top-up when new position liquidity already meets the target', async () => {
    const { sdkService, monitor, config, createAddLiquidityFixTokenPayload } = makeTopUpScenario({
      newPositionLiquidity: '5000000', // equals storedLiquidity — no top-up needed
      topUpBalanceA: '500000',
      topUpBalanceB: '500000',
    });
    ...
    // No top-up — only the initial add-liquidity call.
    expect(createAddLiquidityFixTokenPayload).toHaveBeenCalledTimes(1);
  });

  it('skips top-up when new position liquidity exceeds the target', async () => {
    const { sdkService, monitor, config, createAddLiquidityFixTokenPayload } = makeTopUpScenario({
      newPositionLiquidity: '6000000', // exceeds storedLiquidity — definitely no top-up
      topUpBalanceA: '500000',
      topUpBalanceB: '500000',
    });
  ...
  it('skips top-up and still succeeds when no wallet funds remain after initial deposit', async () => {
    const { sdkService, monitor, config, createAddLiquidityFixTokenPayload } = makeTopUpScenario({
      newPositionLiquidity: '4000000', // below target
      topUpBalanceA: '0',             // no A remaining
      topUpBalanceB: '0',             // no B remaining
    });

    const svc = new RebalanceService(sdkService, monitor, config);
    const result = await svc.checkAndRebalance('0xpool');

    // Rebalance must still succeed even though top-up had no funds.
    expect(result).not.toBeNull();
    expect(result!.success).toBe(true);

    // Only the initial add-liquidity call — no top-up.
    expect(createAddLiquidityFixTokenPayload).toHaveBeenCalledTimes(1);
  });

  it('returns positionId from openNewPosition so topUpLiquidityIfNeeded can locate the position', async () => {
    const { sdkService, monitor, config, createAddLiquidityFixTokenPayload } = makeTopUpScenario({
      newPositionLiquidity: '4000000',
      topUpBalanceA: '500000',
      topUpBalanceB: '500000',
    });

    const svc = new RebalanceService(sdkService, monitor, config);
    await svc.checkAndRebalance('0xpool');

    // The third getPositions call (from topUpLiquidityIfNeeded) must have been made
    // with the wallet address — confirming openNewPosition forwarded the position ID.
